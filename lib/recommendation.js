...
export async function getUniversityRecommendations() {
  ...
  try {
    ...
    // 2. Lade Benutzer-Lieblingsfächer
    ...
    // 3. Lade bereits gematchte Universitäten
    ...
    // 4. Finde Universitäten, die die favorisierten Fächer anbieten
    ...
    // 5. Sortiere Unis nach Anzahl der Übereinstimmungen, filtere bereits gematchte heraus
    ...
    // 6. Lade vollständige Universitätsdaten
    ...
    // 7. Lade die Fächerdaten für alle involvierten Fächer
    ...
    // 8. Kombiniere die Daten und sortiere nach Relevanz
    const sortedUniversities = await Promise.all(universities
      .sort((a, b) => uniMatches[b.id] - uniMatches[a.id])
      .map(async (uni) => {
        // Finde alle Fächer dieser Universität, die für den Benutzer relevant sind
        const uniSubjects = uniSubjectDetails[uni.id]?.map(detail => {
    ...
    return await enhanceUniversityImages(sortedUniversities);
  } catch (error) {
 ...
 */
export async function saveUniversityMatch(universityId) {
  const supabase = createClient();
